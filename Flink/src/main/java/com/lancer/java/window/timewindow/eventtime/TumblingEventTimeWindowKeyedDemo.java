...
public class TumblingEventTimeWindowKeyedDemo {
    ...
    private static void test02(StreamExecutionEnvironment env) {
        ...
        SingleOutputStreamOperator<Tuple3<Long, String, Integer>> linesWithWaterMark = mapDataStream.assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor<Tuple3<Long, String, Integer>>(Time.seconds(0)) { // 达到WaterMark时，延迟Time.seconds(s)后触发
            @Override
    ...
    public static void test03(StreamExecutionEnvironment env) throws IOException {
        ...
        DataStreamSource<String> lineWithWaterMark = env.fromSource(kafkaSource, WatermarkStrategy.<String>forBoundedOutOfOrderness(Duration.ofMillis(0)).withTimestampAssigner((element, recordTimestamp) -> Long.parseLong(element.split(",")[0].trim())), "kafkaSource");
