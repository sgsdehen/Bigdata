```scala
1、transformation与DataFlow中的operator一般是一一对应的关系（没有Operator Chain），经过优化之后，一个transformation也可以对应多个operator。

2、程序中，默认使用了Operator Chain的技术进行优化，将Parallelism一样，且没有发生全局聚合的算子合并成一个Task，组成一个transformation。

3、只有在DataStream中，Operator Chain可以手动屏蔽（env.disableOperatorChaining();）
    
4、使用Operator Chain的优点：
    (1) 减少线程间的切换
    (2) 减少序列化和反序列化的操作
    (3) 降低了延迟和提高了吞吐量

5、如何组成Operator Chain：
    (1) parallelism一样
    (2) 算子之间的关系是one-to-one，不是redistrubuting的关系

6、使用.startNewChain();开启一个新的Operator Chain

7、one-to-one的情况下，Parallelism相同，一个transformation到另一个transformation为FORWARD（开启新operator chain时发生）,不同则为REBALANCE；Redistributing的情况下为HASH;
```
...
~~~java
Sink到Kafka的时候:
streams.addSink(new FlinkKafkaProducer[](topic, (element, timestamp) -> new ProducerRecord<>(topic, element.getBytes(StandardCharsets.UTF_8)), producer, FlinkKafkaProducer.Semantic.EXACTLY_ONCE));
// Flink version = 1.14时
streams.sinkTo([KafkaSink<>.class])

若是指定了EXACTLY_ONCE语义，需要开启checkpoint机制
FlinkKafkaProducer.Semantic.EXACTLY_ONCE ====> env.enableCheckpointing(2000)

设置参数transaction.timeout.ms
~~~
...
~~~java
广播变量作用：
	将用于只读的数据集封装成广播变量，分发到各个TaskManager进程对应的内存中，可以被TaskManager进程中所有的solt所维护的线程所共享，节省了内存开销。
	两个数据集进行连接操作时，一个数据集较小，另外一个数据集较大，此时容易产生数据倾斜；若用广播变量，可以将reduce的shuffle操作提前到map阶段，从而避免数据倾斜。

一小一大，适合广播，差不多大小，使用join，但是效率比广播低；
~~~
...
分布式文件缓存：DistributedCache
	...
	（将HDFS上的资源文件瞎子到本地，即TaskManager进程所在的节点 -> TaskManager进程中的slot所管理的线程读取本地文件中的数据，并装载到内存 -> 进行计算）
...
~~~scala
键控状态：KeyedState
	KeyedStream，keyBy算子对应的状态就是KeyedState
	KeyedState底层的数据结构有：ValueState、ListState、Reducing State、AggregationState及MapState
	
获取状态(在RichxxxxFunction中):
	val xxxState = this.getRuntimeContext.getState[](xxxStateDescriptor)
获取状态中的值：
	xxxState.value()
更新状态中的值：
	xxxState.update(value)
...
~~~scala
状态后端：FsStateBackend
	算子的状态值保存在TaskManager进程的内存中
	Checkpoint中保存的是算子的状态值，以及当前的Flink应用迄今为止实时计算的结果
...
~~~
滚动计时窗口：
	timeWindow(Time.seconds(4)) // 每隔4s，统计
滑动计时窗口：
	timeWindow(Time.seconds(4), Time.seconds(2)) // 每间隔2s，统计过去4s
...
~~~
滚动计数窗口：
	countWindow(3) // 每统计3个，算一个窗口，然后进行后续计算
滑动计数窗口：
	countWindow(4, 2) // 每经过2个，统计过去4个，然后进行后续计算
~~~
...
窗口函数：
	增量聚合函数：
		窗口中的元素从第二个开始，每进入一个元素，就会执行一次；然后将迄今为止执行的结果与新的元素进行比较，获得最新的值，直到窗口关闭，出发sink操作
		适用于窗口中元素体量大，否则，容易导致OOM
		不需要记录窗口中所有的元素，只需要保存迄今为止增量计算的结果即可
	
	全窗口函数：
		等到窗口关闭口，窗口中的所有元素一起进行计算，触发sink操作
		适用于窗口中元素体量小，编程简洁
		需要通过StateBackend记录敞口中所有的元素，待到窗口关闭，一起进行计算。
~~~
...
![image-20211002225050579](./image/image-20211002225050579.png)

~~~
SparkStreaming用的是系统时间，Flink支持event事件时间，结合Watermark处理乱序数据

TaskManager中的task slot只是对内存资源进行了划分隔离，对cores没有隔离，多个task slot共享cores。

task slot的个数和core的个数是一一对应，若core支持超线程，则task slot = 2 * core

spark中，--executor-cores 2指定的只是Worker中能启动多少个Executor

TaskManager会向JobManager注册slot的个数

spark中，Master管资源，Driver管任务，而Flink中JobManager = 资源调度 + 任务调度

将多个Operator优化在一起称为一个Task，从算子角度来看，也叫做一个operator chain；一个Task可以由多个线程执行，每一个线程执行的Task叫做该Task的一个subTask，而subTask的数量由线程数（并行度）决定；实际上，在自动优化后，单个的operator也叫做一个Task，因为并行度一样的one-to-one或Redistributing都会合并成一个Task，而剩下的operator就是其中并行度不一样（会发生rebalance）的
~~~
...
![Flink02-Standalone YarnHA](./image/Flink02-Standalone YarnHA.png)

~~~
一、
	Flink On Yarn，本质上是在Yarn上启动了一个Flink集群
...
三、
	Spark中的Master、和Worker角色是用于资源管理的，被Yarn中的角色替代了（RM -> Master、NM -> Worker）,在Yarn中只存在任务管理的角色Driver和		Executor。而Flink跟Spark不一样，JobManager和TaskManager所要做的事情太多了,所以都存在于Yarn中。
...
五、
	linux环境下，启动yarn-session或提交任务，不需要去配置flink，只需要在lib下有hadoop插件的jar包就可以使用flink on yarn；IDEA中运行，则需要有Hadoop的依赖；可以将hadoop依赖打入jar包中，然后放到linux中运行。


六、
	yarn是细粒度的资源调度，需要多少就给多少，所以在yarn-session启动时，-n指定的TaskManager的个数不会一次性给出；
...
![Flink03-subtask划分及任务调度](./image/Flink03-subtask划分及任务调度.png)
...
四、
	disableChaining()和startNewChain区别：
		...
		拿WordCount说：在flatMap()上，使用disableChaining()是不让flatMap()算子存在于source + flatMap + Map的operator chain中，而是让其独立开来，所以将一个Task转换成3个Task；使用startNewChain()是从flatMap()算子开始新建一条operator chain，使source和flatMap之间断开，让flatMap和Map组成新的Task.
...
![image-20211030223851665](./image/image-20211030223851665.png)

~~~
状态是下游中每一个TM中的subtask共享一个状态，而广播数据是向每一个subtask中发送一个。
~~~
...
processFunction:
	可以更加灵活的对数据进行处理
	可以获取KeyedStream的状态，并且对状态进行编程
	对KeyedStream可以定义定时器，可以按照指定的时间对数据进行批量处理或增量聚合
