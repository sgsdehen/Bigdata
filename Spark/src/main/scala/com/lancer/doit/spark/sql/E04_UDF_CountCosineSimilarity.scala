...
object E04_UDF_CountCosineSimilarity {
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkEnvUtils.getEnv("session", logLevel = "error", conf = conf).asInstanceOf[SparkSession]
    ...
    summaryT.select($"id", $"uid", cos_sim(col("features"), summaryT("uFeatures")) as "cos_similar").show(100, truncate = false)
